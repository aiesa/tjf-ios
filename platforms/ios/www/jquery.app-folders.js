...
// ==============
			...
			$(".close-folder").bind("click", function(event) {
				folder_container.toggleClass("active-tool");
				...
				$(" .jaf-container").find(".folder").not(clickedFolder).each(function() {
					if (!folderContentShown) {
						$(this).transition({ opacity: 0.20 }, settings.animationSpeed);
					}
					else {
						$(this).transition({ opacity: 1.00 }, settings.animationSpeed);
					}
				});

				document.location.hash = settings.defaulthash;
				
				//Reset the margin-top for the container
				folder_container.parent().parent().transition({ marginTop: settings.marginTopBase }, settings.animationSpeed );
			});
			$(".folder").bind("click", function(event) {
				...
				if ($(" .jaf-container .active-tool").length === 0){
					...
					$(" .jaf-container").find(".folder").not(clickedFolder).each(function() {
						if (!folderContentShown) {
							$(this).transition({ opacity: settings.opacity }, settings.animationSpeed);
						}
						else {
							$(this).transition({ opacity: 1.00 }, settings.animationSpeed);
						}
...
// ==============
					...
					// To enable shifting of the rows' top margin on click (works best with overflow: hidden):
						...
						$(this).parent().parent().transition({ marginTop: marTop }, settings.animationSpeed );
					}
...
//--to keep it from scrolling to it
					...
					document.location.hash = settings.defaulthash+"/"+hash;
					if ( node.length ) {
						node.attr( 'id', hash );
					}
				...
				//If there IS a currently displayed tool details area, CLOSE IT
					...
					if (folderContentShown) {
						...
						//Reset the margin-top for the container
						$(this).parent().parent().transition({ marginTop: settings.marginTopBase }, settings.animationSpeed );
						...
						if (settings.instaSwitch !== false) {
							...
							$(' .jaf-container .folder').transition({ opacity: 1.00 }, speed);
							...
							$(" .jaf-container").find(".folder").not(clickedFolder).each(function() {
								if (!folderContentShown) {
									$(this).transition({ opacity: settings.opacity }, speed);
								}
								else {
									$(this).transition({ opacity: 1.00 }, speed);
								}
							});
							...
							$('.active-tool').removeClass('active-tool');
							$('.jaf-container .folder').transition({ opacity: 1.00 }, settings.animationSpeed);
			...
			$('.close-folder').click(function(){
				...
				$('.folderContent').transition({"height":'0px' }, settings.animationSpeed).hide().css({"height":"auto"});
				$('.active-tool').removeClass('active-tool');
				$('.jaf-container .folder').transition({ opacity: 1.00 }, settings.animationSpeed);							
...
// ==============
			...
			var clickedFolder = $(location.hash.replace(/^.*?#/, '').split('/')[1]),
			...
			if ($i != -1) {
				var marTop = settings.marginTopBase - (settings.marginTopIncrement * ($i))
				$(this).transition({ marginTop: marTop }, settings.animationSpeed );
							
				// Don't scroll to the linked item
				$('body').transition({scrollTop:0}, 200, 'linear');
			}
